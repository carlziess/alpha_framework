...
class Json
{
	 ...
	 * @return mixed
	 */
	static public function decode($str, $toArray = true, $charset = 'utf8') {
		...
		return Convert::convert($str, $charset, 'utf8');
	}
	...
	static protected function jsonToString($string) {
		...
		for ($c = 0, $length = strlen($chrs); $c < $length; ++$c) {
			...
			if ('\b' == $compare) {
				...
				$decodeStr .= Convert::utf16beToUTF8($utf16); //self::utf16beToUTF8($utf16);
				$c += 5;
	 ...
	 * @return string
	 */
	static protected function stringToJson($string, $charset = 'utf-8') {
		$string = Convert::convert($string, 'utf-8', $charset);
		$ascii = '';
		...
		for ($c = 0; $c < $strlen; ++$c) {
			...
			if (0x08 == $ordVar) {
				...
				$ascii .= sprintf('\u%04s', bin2hex(Convert::utf8ToUTF16BE($char)));
	 ...
	 * @return string
	 */
	static protected function arrayToJson(array $array, $charset = 'utf-8') {
		if (is_array($array) && count($array) && (array_keys($array) !== range(0, sizeof($array) - 1))) {
			array_walk($array, array('LeafJson', '_nameValue'), $charset);
			return '{' . join(',', $array) . '}';
		}
		array_walk($array, array('LeafJson', '_value'), $charset);
		return '[' . join(',', $array) . ']';
	}
	...
	static protected function objectToJson($object, $charset = 'utf-8') {
		...
		array_walk($vars, array('LeafJson', '_nameValue'), $charset);
		return '{' . join(',', $vars) . '}';
	}
